...
class Player:
    ...
    # ── Private method shims (test backward compat — Phase 1) ────────────────

    def _compute_dice_stats(self) -> None:
        if isinstance(self._strategy, CPUStrategy):
            self._strategy._compute_dice_stats(self.dice, self.num_dice)
            self.rolls_mode = self._strategy._rolls_mode
            self.mode_count = self._strategy._mode_count

    def _make_opening_bid(self) -> TurnResult:
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._make_opening_bid(self.name, self.dice, self.num_dice)

    def _build_response_context(self, prev_event, tot_other_dice, bidder_num_dice, all_prev_bids):
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._build_response_context(
            self.dice, self.num_dice, prev_event, tot_other_dice, bidder_num_dice, all_prev_bids)

    def _compute_challenge_probability(self, bidder_name, tot_other_dice, bidder_num_dice, needed):
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._compute_challenge_probability(
            bidder_name, tot_other_dice, bidder_num_dice, needed)

    def _effective_challenge_threshold(self, bidder_name):
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._effective_challenge_threshold(bidder_name)

    def _get_permissible_bids(self, prev_bid_cnt, tot_other_dice, all_prev_bids):
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._get_permissible_bids(prev_bid_cnt, tot_other_dice, all_prev_bids)

    def _rank_and_select_bid(self, permissible, model, all_prev_bids):
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._rank_and_select_bid(
            self.dice, self.num_dice, permissible, model, all_prev_bids)

    def _decide_action(self, ctx) -> TurnResult:
        assert isinstance(self._strategy, CPUStrategy)
        return self._strategy._decide_action(self.name, ctx)
